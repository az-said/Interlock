...
5. **Recovery never guesses.** It acts by what the target can do: retry if the target dedupes, look up if it can be queried, and otherwise say `AMBIGUOUS` out loud.
...
The output of every effect is a **receipt**: four booleans and a final state.
...
$100 paid, one $20 partial refund approved. Invariant: exactly $20 refunded, or $0 if the lease was revoked or the order became ineligible first. Two baselines: today's naive re-run, and the conventional "just use idempotency keys" durable operation.

| fault | naive (today) | idempotency key only | gate @ tier 1 | gate @ tier 2 | gate @ tier 3 |
|---|---|---|---|---|---|
| crash before send | $20 ✅ | $20 ✅ | $20 ✅ | $20 ✅ | **$0, `AMBIGUOUS`** ⚠️ |
| crash before ack | **$40** ❌ | $20 ✅ | $20 ✅ | $20 ✅ | $20, `AMBIGUOUS` ⚠️ |
| duplicate submit | **$40** ❌ | $20 ✅ | $20 ✅ | $20 ✅ | $20 ✅ |
| model re-decides $30 on retry | **$50** ❌ | $20 ✅ | $20, refused ✅ | $20, refused ✅ | $20, refused ✅ |
| same request, different amount | **$50** ❌ | $20 ✅ | refused ✅ | refused ✅ | refused ✅ |
| lease revoked mid-flight | refunded ❌ | **refunded** ❌ | refused ✅ | refused ✅ | refused ✅ |
| order became ineligible | refunded ❌ | **refunded** ❌ | refused ✅ | refused ✅ | refused ✅ |
...
**"CaMeL solves prompt injection with capabilities."** Yes, and it's the closest prior work. CaMeL controls whether untrusted data can influence control flow. It has no model of crashes, retries, duplicate delivery, or revocation. It answers "was this action allowed?" Interlock answers "did this allowed action happen, once, under authority that was live when it fired?"
...
**Not built yet.** Real Stripe and GitHub targets. A journal that isn't a local file. Premise extraction beyond file hashes and function arity. A notary that both sides write to, so tier 3 services can be lifted to tier 2 without changing their API.
...
interlock/           the runtime
  ...
  gate.py            six invariants, recovery by tier, claims registry, two baselines
  targets/
...
- Debenedetti et al. (2025). CaMeL. arXiv 2503.18813. The closest prior work; no failure model.
