...
- **Tested against ourselves.** 182 tests including a 2,000-case randomized fault sweep. Writing them found 15 real bugs in our own gate; each fix has a test that failed before it.
...
**Explained.** A refusal says exactly what changed and what would still be safe. A $50 refund on a $100 order, after support refunded $30 by hand, comes back as `changes: [{"field": "refunded", "was": 0, "now": 30}]` with the repair `still_fits` (70 is left to refund). A $100 request after the same hand refund suggests `refund_remaining`, `{"amount": 70}`. Targets opt in with an optional `explain()` method; targets without it keep refusing with plain strings. The same structure rides on the MCP proxy refusal (`_meta.interlock.escalation`) and the Temporal helper's error. A repair is a new decision: a new effect id, never the refused one with a different payload, back through the rules or a person, and superseded if the facts moved again.
...
interlock/           the runtime
  ...
  approvals.py       rules, routes with SLAs, a queue rebuilt from the journal, approvals as authority re-checked at send time
