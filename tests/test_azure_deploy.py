...
class AzureDeployScript(unittest.TestCase):
    ...
    def make_repo(self, reads_names=True):
        d = tempfile.mkdtemp()
        ...
        names = ("INTERLOCK_PUBLIC INTERLOCK_ALLOWED_HOSTS INTERLOCK_TRUSTED_PROXY_HOPS "
                 "INTERLOCK_RATE_LIMIT_PER_MINUTE INTERLOCK_RATE_LIMIT_RUNS_PER_HOUR")
        with open(os.path.join(d, "backend", "api.py"), "w") as f:
            f.write(f"# {names if reads_names else 'INTERLOCK_PUBLIC'}\n")
    ...
    def deploy(self, repo, fqdn, app_missing, role_id=""):
        ...
        p = self.run_script(cwd=repo, path=os.path.join(repo, "bin") + os.pathsep + BASE_PATH,
                            ...
                            STUB_FQDN=fqdn, STUB_APP_MISSING="1" if app_missing else "0", STUB_ROLE_ID=role_id)
        calls = ""
    ...
    def test_refuses_env_names_the_server_does_not_read(self):
        ...
        self.assertNotEqual(p.returncode, 0)
        self.assertIn("INTERLOCK_RATE_LIMIT_PER_MINUTE", p.stderr)
