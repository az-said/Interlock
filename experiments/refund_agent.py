...
from interlock import Gate, Naive, IdempotencyOnly, Leases, SimulatedCrash
...
def run(system, tier, fault):
    ...
    def recover_or_retry(retry_proposal):
        rec = s.recover()
    ...
    expect = 0 if fault in ("lease_revoked", "stale_eligibility") else AMOUNT
    ambiguous = str(out) == "AMBIGUOUS"
    held = (total == expect) or (ambiguous and total <= expect)
    caveat = "liveness_lost" if ambiguous else ""
    return {"outcome": str(out), "refunded": f"${total}", "invariant_held": held, "caveat": caveat}

SYSTEMS = [("naive", 3, "naive"), ("idempotency", 1, "idempotency@tier1"),
