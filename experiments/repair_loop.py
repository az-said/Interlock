...
it decides, calls the tool, and on a refusal it may read the approval again and decide again, up to
3 attempts. What happens to each order:

    routine        nothing goes wrong
    partial_hand   support refunds $5 to $15 by hand after the agent decided, before the send
    full_hand      support refunds the whole $20 by hand after the agent decided
...
Three ways to run the same day, against the same payments service (no dedup, refunds can be looked up):
    ...
    interlock+repair   the same, plus an approval: a refusal says what changed, and a corrected call
                       that fits the approval is sent, once
...
SYSTEMS = ("no gate", "interlock", "interlock+repair")


class Shop:
    """
    Payments with no dedup; each order's case approves $20 in total. A refund in `by_hand` lands after the
    agent's decision and before its send: on the order's second read (the gate's re-check), or when the
    refund call arrives, whichever comes first. An order in `crash` commits and then loses the answer.
    """
    def __init__(self):
