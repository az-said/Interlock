...
| **MCP** | `python3 -m interlock.mcp_proxy --config ... -- <server command>`, no agent code change (`interlock/mcp_proxy.py`) | Tests run the proxy as a real process in front of a fake MCP server (`tests/fake_mcp_server.py`): the proxy killed mid-call recovers once on restart, with or without the `initialize` handshake (a client on the stateless 2026-07-28 spec is recovered before its first gated call), and a hand refund during the outage is refused even when the agent retries (`tests/test_mcp_proxy.py`). One optional test runs a stateless client's gated call against a real mcp 2.x server; the crash and recovery tests use the fake |
